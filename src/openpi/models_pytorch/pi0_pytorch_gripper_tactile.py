...
class PI0PytorchWithGripperTactile(nn.Module):
    ...
    def forward(self, observation, actions, tactile_history, tactile_future, noise=None, time=None) -> Tensor:
        ...
        tactile_emb, tactile_pad_masks, tactile_att_masks = self.embed_tactile(tactile_data)
        ...
        if (
            ...
            tactile_emb = tactile_emb.to(dtype=torch.bfloat16)

        # 5. Mask Construction (순서: Prefix -> Tactile -> Suffix 라고 가정)
        # 실제 모델에 들어가는 리스트 순서: [prefix_embs, tac_embs, suffix_embs]

        pad_masks = torch.cat([prefix_pad_masks, tactile_pad_masks, suffix_pad_masks], dim=1)
        att_masks = torch.cat([prefix_att_masks, tactile_att_masks, suffix_att_masks], dim=1)
        ...
        def forward_func(prefix_embs, tactile_emb, suffix_embs, att_2d_masks_4d, position_ids, adarms_cond):
            outputs, _ = self.paligemma_with_expert.forward(
                attention_mask=att_2d_masks_4d,
                position_ids=position_ids,
                past_key_values=None,
                inputs_embeds=[prefix_embs, tactile_emb, suffix_embs],
                use_cache=False,
                adarms_cond=[None, None, adarms_cond],
            )
            # outputs: [prefix_out, tactile_out, suffix_out]
            return outputs[2]

        suffix_out = self._apply_checkpoint(
            forward_func, prefix_embs, tactile_emb, suffix_embs, att_2d_masks_4d, position_ids, adarms_cond
        )
