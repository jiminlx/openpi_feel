...
class Pi0JaxWithFrankaTorque(_model.BaseModel):
    def __init__(self, config: pi0_config.Pi0Config, rngs: nnx.Rngs):
        super().__init__(config.action_dim, config.action_horizon, config.max_token_len)
        self.pi05 = config.pi05
        self.torque_dim = config.torque_input_dim
        ...
        llm = nnx_bridge.ToNNX(
            _gemma.Module(
                configs=[paligemma_config, action_expert_config, torque_expert_config],
    ...
    def embed_torque(
        ...
        """Embeds Torque Stream: History (Static) + Future (Noisy & Time-Conditioned)"""
        input_mask = []
        ar_mask = []
        tokens = []
        ...
        torque_future_tokens = self.torque_in_proj(noisy_torque_future)
        ...
        if self.pi05:
            # Time MLP for adaRMS
            time_emb = self.torque_time_mlp_in(time_emb)
            time_emb = nnx.swish(time_emb)
            time_emb = self.torque_time_mlp_out(time_emb)
            time_emb = nnx.swish(time_emb)
        ...
        tokens.append(torque_future_tokens)
        input_mask.append(jnp.ones(torque_future_tokens.shape[:2], dtype=jnp.bool_))
        
        # Attention Mask: [True] (New Block) + [False] (Internal Attention)
        # PyTorch: [1] + [0] * (seq_len - 1)
        ar_mask += [True] + ([False] * (torque_future_tokens.shape[1] - 1))
    ...
    def compute_loss(
        ...
        **kwargs
        ...
        preprocess_rng, noise_rng_act, noise_rng_torque, time_rng = jax.random.split(rng, 4)
        ...
        # 1. Noise Generation (Action & Torque)
        noise_act = jax.random.normal(noise_rng_act, actions.shape)
        noise_torque = jax.random.normal(noise_rng_torque, torque_future.shape)
        ...
        x_t_torque = time_expanded * noise_torque + (1 - time_expanded) * torque_future
        
        # Targets
        u_t_act = noise_act - actions
        u_t_torque = noise_torque - torque_future

        # 2. Embedding Streams
        # Stream 1: Prefix
        prefix_tokens, prefix_mask, prefix_ar_mask = self.embed_prefix(observation)
        
        # Stream 2: Torque (History + Future)
        # Assuming observation has torque_history
        torque_tokens, torque_mask, torque_ar_mask, adarms_torque = self.embed_torque(
            observation.torque_history, x_t_torque, time
        )
        ...
        # 3. Combine & Mask
        # Order: [Prefix, Torque, Action]
        input_mask = jnp.concatenate([prefix_mask, torque_mask, action_mask], axis=1)
        ar_mask = jnp.concatenate([prefix_ar_mask, torque_ar_mask, action_ar_mask], axis=0)
        ...
        (prefix_out, torque_out, action_out), _ = self.PaliGemma.llm(
            [prefix_tokens, torque_tokens, action_tokens], 
            mask=attn_mask, 
            positions=positions, 
            adarms_cond=[None, adarms_torque, adarms_act]
        )

        # 5. Projection & Loss
        # Torque Output: Only last 'future_len' tokens
        future_len = torque_future.shape[1]
        v_t_torque = self.torque_out_proj(torque_out[:, -future_len:])
        
        # Action Output
        v_t_act = self.action_out_proj(action_out[:, -self.action_horizon:])

        loss_torque = jnp.mean(jnp.square(v_t_torque - u_t_torque), axis=-1).mean(axis=-1)
        loss_act = jnp.mean(jnp.square(v_t_act - u_t_act), axis=-1).mean(axis=-1)
        
        # Weighting (assuming 0.1 from PyTorch code)
        w_torque = 0.1
        
        return loss_torque * w_torque + loss_act
    ...
    def sample_actions(
        ...
        *,
        ...
        noise_torque: at.Float[at.Array, "b fh td"] | None = None,
        ...
        if noise_torque is None:
            # Assuming torque horizon same as action horizon or defined in config
        ...
        # Denoising Loop
        def step(carry):
            x_t_act, x_t_torque, time = carry
            
            # Embed Dynamic parts
            # Note: Torque History is re-embedded here with Future. 
            # Optimization: Could cache history embedding, but it's cheap compared to LLM.
            
            # Torque Stream Embedding (History + Future)
            torque_tokens, torque_mask, torque_ar_mask, adarms_torque = self.embed_torque(
                observation.torque_history, x_t_torque, jnp.broadcast_to(time, batch_size)
            )
            ...
            # Generation block = [Torque, Action]
            gen_mask = jnp.concatenate([torque_mask, action_mask], axis=1)
            gen_ar_mask = jnp.concatenate([torque_ar_mask, action_ar_mask], axis=0)
            ...
            (prefix_out, torque_out, action_out), _ = self.PaliGemma.llm(
                [None, torque_tokens, action_tokens],
                mask=full_attn_mask,
                positions=positions,
                kv_cache=kv_cache,
                adarms_cond=[None, adarms_torque, adarms_act],
            ...
            # Project Outputs
            v_t_torque = self.torque_out_proj(torque_out[:, -noise_torque.shape[1]:])
            v_t_act = self.action_out_proj(action_out[:, -self.action_horizon:])

            # Euler Step
            next_x_act = x_t_act + dt * v_t_act
            next_x_torque = x_t_torque + dt * v_t_torque
            
            return next_x_act, next_x_torque, time + dt

        def cond(carry):
            _, _, time = carry
            return time >= -dt / 2

        x_0_act, x_0_torque, _ = jax.lax.while_loop(cond, step, (noise_action, noise_torque, 1.0))
        
        return x_0_act, x_0_torque
